...
let app = new Vue ({
	 ...
	 * Задание 1 
	 * 
	 * Если делать через методы то необходимо будет вешать дополнительный обработчик на клик кнопки
	 * или на input вешать @keyup.enter="filter()" чтобы срабатывало по enter
	 * создавать в data дополнительный массив filtered:[]
	 * заполнять массив в mounted
	 * и создать метод фильтрации
	 * filter(){
	 *  this.filtered = this.products.filter(e => ~e.product_name.toLowerCase().indexOf(this.search.toLowerCase()))
	 * }
	 * 
	 * Но т.к. лень двигатель прогресса, то проще и удобнее сделать через вычисляемое свойство
	 * фильтрация продуктов по вводу названия 
	 * 
	 */
	computed:{
		filtered(){
